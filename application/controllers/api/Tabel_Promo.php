...
class Tabel_Promo extends RestController
{

    public function index_get($id = 0)
    {
        $check_data = $this->db->get_where('tb_promo', ['id' => $id])->row_array();

        if ($id) {
            if ($check_data) {
                $data = $this->db->get_where('tb_promo', ['id' => $id])->row_array();
    ...
    public function index_post()
    {
        ...
        $insert = $this->db->insert('tb_promo', $data);
